...
--  if FORCE_LANDSCAPE and win_w < win_h then
--    win_w, win_h = win_h, win_w
--    VIRTUAL_LANDSCAPE = true
--  else
--    VIRTUAL_LANDSCAPE = false
--  end
...
local function screen_shader(shader_code)
  ...
  local pre2 = " extern vec2 SCREEN_SIZE; extern vec3 PALETTE[256]; extern int SWAPS[256];"
  
  local i, li = 1
  repeat
    li, _, i = i, shader_code:find("extern%sImage.-;", i)
  until i == nil
  
  if li > 1 then
    shader_code = pre..shader_code:sub(1, li)..pre2..shader_code:sub(li+1, #shader_code)..after
  else
    shader_code = pre..pre2..shader_code..after
  end
...
local function use_palette(plt)
  ...
  for i = 0, #plt do
    ...
    _shader_colors[i+1] = col[1]*100 + col[2]*10 + col[3]
  end
...
local function new_surface(w, h, key)
  ...
  _D.surf_list[key] = love.graphics.newCanvas(w, h, {type = "2d", format = "rgba8", dpiscale = 1})
